...
void liftArmTest()
{
  ...
  while(abs(angleVal1) < 86) {mArm1 = -50; getJoystickSettings(joystick); mArmTurn = joystick.joy1_y1;} mArm1 = 0; //95
  mArmTurn = 0;
  while(!btn(3)) getJoystickSettings(joystick);
  //wait1Msec(750);
  while(abs(angleVal2) < 060) {mArm2 = -80; getJoystickSettings(joystick); mArmTurn = joystick.joy1_y1;} mArm2 = 0; //60
  while(abs(angleVal2) < 085) {mArm2 = -50; getJoystickSettings(joystick); mArmTurn = joystick.joy1_y1;} mArm2 = 0; //85
  while(abs(angleVal2) < 092) {mArm2 = -15; getJoystickSettings(joystick); mArmTurn = joystick.joy1_y1;} mArm2 = 0; //100
  mArmTurn = 0;
  while(!btn(3)) getJoystickSettings(joystick);
  //wait1Msec(500);
  while(abs(angleVal1) < 105) {mArm1 = -50; getJoystickSettings(joystick); mArmTurn = joystick.joy1_y1;} mArm1 = 0; //115
  mArmTurn = 0;
  while(!btn(3)) getJoystickSettings(joystick);
  //wait1Msec(500);
  //while(abs(angleVal2) < 110) {mArm2 = -30; getJoystickSettings(joystick); mArmTurn = joystick.joy1_y1;} mArm2 = 0; //120

  wait1Msec(500);
  while(abs(angleVal3) < 20) {mArm3 = 40; getJoystickSettings(joystick); mArmTurn = joystick.joy1_y1;} mArm3 = 0; //72
...
void buttonControl()
{
  ...
  /* RB */ if(btn(5)) {mArmFlappyThing = 40; lastPressed45 = 5;}
  /*    */ if(!btn(4) && !btn(5))
              {
                if(lastPressed45 == 5) {mArmFlappyThing = 25;}
                else {mArmFlappyThing = 0;}
  ...
  /*LTRT*/ if(btn(6)) {i--; nMotorEncoder[motorArm3] = 0; nMotorEncoder[motorArmTurn] = 0;}
           ...
           mArmTurn = -i*100;
           if(btn(9)) nMotorEncoder[motorArmFlappyThing] = 0;
  /*Back*/ //if(btn(8)) {}
