...
task main()
{
  ...
  while(true)                            // Infinite loop:
  {
    ...
    if(xB) {motor[motorA] = 100;}
    else {motor[motorA] = 0;}
    getJoystickSettings(joystick);
    if(((abs(joy1Y1_LV) > threshold)))   // If the right analog stick's Y-axis readings are either above or below the threshold:
    {
      motor[motorD] = joy1Y1_LV / (2*sensetivity);
      nxtDisplayTextLine(1, ""+joy1Y1_LV);// Motor B is assigned a power level equal to the right analog stick's Y-axis reading.
      nxtDisplayTextLine(2, "> thresh");
      nxtDisplayTextLine(3, "-");
    }
    else                                    // Else if the readings are within the threshold:
    {
      nxtDisplayTextLine(1, ""+joy1Y1_LV);
      motor[motorD] = 0;
    ...
    if(abs(joy1Y1_LV) > threshold)   // If the left analog stick's Y-axis readings are either above or below the threshold:
    {
      motor[motorE] = joy1Y1_LV / (2*sensetivity);         // Motor C is assigned a power level equal to the left analog stick's Y-axis reading.
    }
    else                                    // Else if the readings are within the threshold:
    {
    ...
    if(abs(joy1X1_LV) > threshold)
    {
      motor[motorE] -= joy1X1_LV / (2*sensetivity);
      motor[motorD] += joy1X1_LV / (2*sensetivity);
    ...
    //nxtDisplayTextLine(2, ""+joy1Y1_LV);
    //nxtDisplayTextLine(3, ""+joy1X1_LV);
    ...
    if(sVal < 0) sVal = 0;
    if(buttonTemp >= 8) /*Y (4) (Top)*/ {buttonTemp -= 8; xB = !xB;}
    //motor[motorA] = 100;
