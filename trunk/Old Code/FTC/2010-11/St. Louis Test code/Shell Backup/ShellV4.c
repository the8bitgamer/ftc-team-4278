...
{
  //float sensetivity = 55;
  float sensetivity = 90;
  if(abs(HTGYROreadRot(turningGyro)) > sensetivity)
  {
    motor[motorsLeft] = 0;
    motor[motorsRight] = 0;
    while(true) {}
...
int gyroLowSpeedSafetyIterationValue = 0;
int sfvmaiv = 0;
...
void gyroLowSpeedSafety()
{
  ...
  if(gyroLowSpeedSafetyIterationValue > 100)
  {
	  if(abs(HTGYROreadRot(turningGyro)) < 25)
	  {
	    motor[motorsLeft] = 0;
	    motor[motorsRight] = 0;
	    while(true) {}
	  }
	} else {gyroLowSpeedSafetyIterationValue = gyroLowSpeedSafetyIterationValue + 1;}
}
...
void roundRobotValues()
{
  robotX = robotX * 10;
  robotY = robotY * 10;
  robotOverallHeading = robotOverallHeading * 10;

  robotX = floor(robotX)/10;
  robotY = floor(robotY)/10;
  robotOverallHeading = floor(robotOverallHeading)/10;
}
...
void exitMoveFunction() //Turning exit needs no values
{
  ...
  gyroLowSpeedSafetyIterationValue = 0;
...
void moveRobot(float xVal, float yVal, bool areCoordinates) //Semi functional?
{
  enterMoveFunction();
  ...
  if(deltaXVal < 0 && deltaYVal < 0)
  {
    deltaAngle += 180;
  } //Compensate for arcTrig domain restriction
