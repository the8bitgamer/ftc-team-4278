...
int HTGYROreturnX1Test(tMUXSensor muxsensor);
int HTGYROreturnY1Test(tMUXSensor muxsensor);
int HTGYRO_sumAllOffsets();
...
int HTGYROreadRot(tMUXSensor muxsensor) {
  ...
  //int x1 = HTSMUXreadAnalogue(muxsensor);
  //int y1 = HTGYRO_offsets[SPORT(muxsensor)][MPORT(muxsensor)];
}

int HTGYROreturnX1Test(tMUXSensor muxsensor) {return HTSMUXreadAnalogue(muxsensor);}
int HTGYROreturnY1Test(tMUXSensor muxsensor) {return HTGYRO_offsets[SPORT(muxsensor)][MPORT(muxsensor)];}
...
int HTGYROstartCal(tSensors link) {
  ...
  // Store new offset
  HTGYRO_offsets[link][0] = (_avgdata / 10);
...
int HTGYROstartCal(tMUXSensor muxsensor) {
  ...
  // Store new offset
  HTGYRO_offsets[SPORT(muxsensor)][MPORT(muxsensor)] = (_avgdata / 10);

//  return HTGYRO_sumAllOffsets();
