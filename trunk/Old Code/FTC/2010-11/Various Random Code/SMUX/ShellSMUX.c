...
float gyroTurnIntegrate()
{
  ...
  //return HTGYROreadRot(msensor_S4_1)/250;
	gyroValue = HTGYROreadRot(msensor_S2_1);
	summationValue = gyroValue/divide;
	wait1Msec(4);
...
void initializeRobot()
{
  ...
  float igmaiffe = HTGYROreturnX1Test(msensor_S2_1) - HTGYROreturnY1Test(msensor_S2_1);
...
bool checkGyroTurn(float gyroValue, float gyroTargetVal)
{
	if(gyroValue < gyroTargetVal) {return false;}
