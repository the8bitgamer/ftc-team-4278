...
#define turnGyroReading HTGYROreadRot(msensor_S4_1)
#define vertGyroReading HTGYROreadRot(msensor_S4_2)
...
void armUp() //moves arm up (neg. = up)
{
        motor[motorArm] = -50;
        wait1Msec(700);
        motor[motorArm] = 0;
...
void armDown() //begins to move arm down. gravity takes care of the rest.
{
	motor[motorArm] = 15;
        wait1Msec(650);
        motor[motorArm] = 0;
...
float forward(float dist)
{
	return (1440*dist)/(2*Pie*wheelRadius); //returns the encoder value to go Dist for wheelRadius (units for dist equiv. to wheelRadius, usually in.)
}
...
bool checkColorSensors(bool colorTargetIsRed, bool colorTargetIsBlue, bool colorTargetIsWhite, bool colorTargetSumVal)
{
	...
	if(colorTargetSumVal == true)
	{
		...
		int green;
		int blue;
		HTCSreadRGB(colorHitechnicRollingGoal, red, green, blue); //Stores RGB values to int red, int green, and int blue. rgb > threshold, stop.
...
void resetHeading(float gyroTargetVal, float gyroValue)
{
	if(!checkGyroTurn(gyroValue, gyroTargetVal))
	{
		...
		while(!checkGyroTurn(gyroValue, gyroTargetVal)) {gyroValue += gyroTurningIntegrate();}
		
		gyroValue += gyroTurningIntegrate;
		resetHeading(gyroTargetVal, gyroValue);
	}
}
...
void moveRobot(float targetDist, float gyroTargetVal, float leftWheelPower, float rightRatioVal, bool colorTargetIsRed, bool colorTargetIsBlue, bool colorTargetIsWhite, bool colorTargetSumVal, int encoderExitByDefault)
{
	...
	while(exitMove == false)
	{
		...
		gyroTurnChange = gyroTurn; //For later use
		...
		if(abs(gyroTurnChange) > safeGyroTurnChange) {freezeRobot();} //If we are knocked out of the safe range for turning, stop.
