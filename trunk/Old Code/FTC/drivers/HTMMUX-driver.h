...
/*
 * $Id: HTMMUX-driver.h 65 2011-09-06 13:52:43Z xander $
 */

#ifndef __HTMMUX_H__
#define __HTMMUX_H__
/** \file HTMMUX-driver.h
 * \brief HiTechnic Motor MUX driver
 *
 * HTMMUX-driver.h provides an API for the HiTechnic Motor MUX driver.
 *
 * Changelog:
 * - 0.1: Initial release
 *
...
ubyte HTMMUXreadStatus(tSensors link, ubyte channel) {
  ...
  if (!writeI2C(link, HTMMUX_I2CRequest, 1))
    return -1;

  if (!readI2C(link, HTMMUX_I2CReply, 1))
    return -1;
...
bool HTMMUXsendCommand(tSensors link, ubyte channel, byte command) {
  ...
  return writeI2C(link, HTMMUX_I2CRequest, 0);
}
...
long HTMMUXMotorEncoder(tSensors link, ubyte channel) {
  ...
  writeI2C(link, HTMMUX_I2CRequest, 4);

  readI2C(link, HTMMUX_I2CReply, 4);
