...
void leftCrate2() {
	//rbtMoveFdTime(-1, 2000); pause(1);  //Realign
	rbtArcRight(96); pause(1);          //Turn to crate
	rbtMoveFdTime(-5, 2000); pause(1);  //Run into crate
	dumpArm(); pause(1);                //Dump arm
	rbtMoveFdTime(1, 2000); pause(1);   //Back up
...
void leftCrate3() {
	//rbtMoveFdTime(-1, 2000); pause(1);  //Relaign
	rbtArcRight(96); pause(1);          //Turn to crate
	rbtMoveFdTime(-5, 2000); pause(1);  //Run into crate
	dumpArm(); pause(1);                //Dump arm
	rbtMoveFdTime(1, 2000); pause(1);   //Back up
...
void runAutoLeft() {
	...
	setLeftMotors(0); setRightMotors(0); pause(3);
	     ...
	     if(rightEncoder < 765)  leftCrate1();
	else if(rightEncoder < 2142) leftCrate2();
	else if(rightEncoder < 3775) leftCrate3();
	else                         leftCrate4();
}
...
int side = 0; int automatic = 0; int crate = -1; int delay = 0;
void makeChoices() {
	nxtDisplayTextLine(0, "NXT: %f", ((float)nAvgBatteryLevel)/1000.0);
	nxtDisplayTextLine(1, "EXT: %f", ((float)externalBatteryAvg)/1000.0);
	...
	while(nNxtButtonPressed != 3) {
		...
		if(nNxtButtonPressed == 1 || nNxtButtonPressed == 2) {
			if(nNxtButtonPressed == 2) side--;
			if(nNxtButtonPressed == 1) side++;
			if(side > 4) side = 0;
			if(side < 0) side = 4;
			while(nNxtButtonPressed == 1 || nNxtButtonPressed == 2) wait1Msec(5);
		}
	} while(nNxtButtonPressed == 3);
	...
	if(side != 2)
		while(nNxtButtonPressed != 3) {
			if(automatic == 0) nxtDisplayTextLine(5, "AUTO: IR");
			else nxtDisplayTextLine(5, "AUTO: Crate %i", automatic);

			if(nNxtButtonPressed == 1 || nNxtButtonPressed == 2) {
				if(nNxtButtonPressed == 2) automatic--;
				if(nNxtButtonPressed == 1) automatic++;
				if(automatic > 4) automatic = 0;
...
task main() {
	...
	wait1Msec(delay);
	if(side == 0) { //Left
		if(automatic == 0) {runAutoLeft(); lockdownRobot();}

		int encDist = 0;
		if(automatic == 1) encDist = 523;
		if(automatic == 2) encDist = 1006;
		if(automatic == 3) encDist = 3277;
		if(automatic == 4) encDist = 4272;
		rbtMoveFdTime(getInchesByEncoder(encDist), 5000);
		if(automatic == 1) leftCrate1();
		if(automatic == 2) leftCrate2();
		if(automatic == 3) leftCrate3();
		if(automatic == 4) leftCrate4();

		lockdownRobot();
	} else if(side == 1) { //Right

	} else if(side == 2) { //Bridge
		moveBridge();
		lockdownRobot();
	}
